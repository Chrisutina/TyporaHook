...
## v2.3.1 — 2026-09-14 —— 公开仓库精简（用户向）

- 🧹 公开仓库只保留**使用端**内容：README / FAQ 移除测试、打包相关表述，文档改为纯用户向
- 🗂 开发者本地内容不再随仓库分发：`tests\`（沙盒）、`tools\test-all.ps1`、`tools\build-release.ps1`、`docs\测试记录.md`（作者本机保留并可继续使用）
- 🎛 工具箱菜单自适应：对应文件缺失时自动隐藏「沙盒自测 / 打发布包」；分析器的后续菜单同逻辑
- 🔧 修正 README 目录树 `tools\` 行重复笔误

## v2.3.0 — 2026-09-14 —— 统一工具箱菜单

- 🎛 新增根目录 `TyporaHook.cmd` + `tools/menu.ps1`：所有命令收进一个窗口，逐条带用途说明、按编号运行 —— **每项跑完都停住等回车**，彻底解决"双击 .ps1 窗口一闪、结果看不到"
- 🧭 菜单覆盖：状态自检 / 部署 / 部署预览 / 升级分析 / 定时器调参 / 沙盒自测 / 回滚 v1 / 还原官方 / 打发布包 / 打开 README·文档·日志·工程目录
- 🛠 组件版本号同步至 v2.3.0（verify / test-all / analyze / build-release）
