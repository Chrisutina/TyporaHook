...
> **版本 v2.3.1**（2026-09-14）· 内含 hook **v2b** · 零联网依赖：部署 / 还原 / 自检全部本地完成 · 仓库：https://github.com/Chrisutina/TyporaHook

把 2026-08 部署的 DreamNya 离线激活 hook，维护成一个"**可部署、可回滚、可自检、可升级适配**"的工程：不修改 Typora 原厂字节码（`atom.compiled.dist.jsc` 原封不动），只替换加载器（`launch.dist.js`）并重打包 `app.asar`；所有改动均可一键还原官方。
