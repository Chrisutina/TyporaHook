...
function Show-Menu {
  ...
  Write-Host '============================================================='
  Write-Host '    TyporaHook 工具箱 v2.3.0   （双击 TyporaHook.cmd 启动）'
  Write-Host '============================================================='
  Write-Host ''
  ...
  Write-Host '  [6] 沙盒自测        — 部署→重复→回滚→还原 全自动四连（不碰真机）'
  Write-Host ''
  Write-Host '  [7] 回滚到 v1 钩子  — 换回 8/25 的 DreamNya 原版钩子'
  Write-Host '  [8] 还原官方        — 一键回官方版（当前版另存为后悔药）'
  Write-Host '  [9] 打发布包        — 生成 release\TyporaHook-v*.zip + SHA256'
  Write-Host ''
...
while ($true) {
  ...
  switch ($c.ToUpper()) {
    ...
    '6' { Invoke-Tool '沙盒自测（test-all）' @('-File', (Join-Path $Root 'tools\test-all.ps1')) }
    ...
    '9' { Invoke-Tool '打发布包' @('-File', (Join-Path $Root 'tools\build-release.ps1')) }
